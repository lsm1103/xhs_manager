...
6. **肖像权红线**：素材关键词的主体必须是**物、场景或抽象视觉**，人不能是画面主体。
   注意这不只是"别写特写词"。下架过的反面例子是这两个词：
     `office worker desk computer`、`person ordering menu counter`
   词本身完全正常，但搜出来必然是真人正脸中景——只要主体是人就会出事。
   要表达"人在做某事"，就换成人的**局部**或人**用的东西**：
     office worker desk computer   →  laptop screen desk office
     person ordering menu counter  →  hands writing order notepad
   允许：hands / back view / silhouette / 无法辨识个人的远景人群(crowd)
   禁止：portrait / headshot / selfie / close-up face，
        以及任何以身份或职业指代人的词（person / worker / student / scientist / chef…）
   另外 over the shoulder **不是**安全表达，它照样能拍到对面人的正脸。
