...
| 入口 | CLI 11 个子命令 | REST 26 个端点 | Web UI（已并入 /tts） |
...
| 配音（原 TTS Studio） | — | **已合并**：同一个 FastAPI，`/tts/*` 是接口，控制台里是「配音」页 |

> 这一节原先的结论是「保持独立进程」，理由是模型要常驻十几 GB、
> 重启代价是分钟级。**这个理由站不住**：重模型从来就不在 Web 进程里，
> indextts2 / omnivoice 各自跑在独立子进程中，注册表只握着管道
> （见 `tts_studio/registry.py`）。合并的代价因此只有一条——
> 主服务退出会带走那些子进程，所以退出时要显式卸载
> （`api.py` 的 shutdown 钩子），否则留下几个 GB 的孤儿进程。

合并后的三件事：

- 路由挂在 `/tts`，形状不变（`/api/models`、`/api/generate`、`/audio/<name>`），
  视频流水线的 HTTP 客户端只改基址。
- 历史从 `data/tts_studio.db` 搬进主库的 `tts_generations` 表，
  搬运走 `python -m xhs_manager.tts_studio.import_legacy`，可重复跑。
- 预热（`TTS_STUDIO_AUTOLOAD`）放后台线程，重模型不挡启动。
...
| **P1** ✅ | 工具体检页 + 配音服务体检 | 几乎不写新逻辑，全是并联已有探测函数 | 已完成 |
