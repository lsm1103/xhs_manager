...
| 期 | 内容 | 为什么是这个顺序 | 估时 |
|---|---|---|---|
| **P0** | 只读界面：任务台 + 任务详情（含视频子流水线）+ 一致性校验 | 先让两条线在同一个界面里可见，不改任何后端行为 | ~1.5 天 |
| **P1** | 工具体检页 + TTS Studio 面板聚合 | 几乎不写新逻辑，全是并联已有探测函数 | ~0.5 天 |
| **P2** | 加外键：把 video 四张表挂到 task / topic_proposal / content_version / publication_plan | 纯 schema 变更 + 回填脚本，存量数据不动 | ~0.5 天 |
| **P3** | 实现 `produce_content` 的 video 分支，6 阶段登记为 `work_item` | 视频线拿到队列、重试、幂等 | ~1 天 |
...
- **存量视频 run 没有 task**：P2 回填时给它们建占位 task（`account_id` 用默认账号），
  或允许 `task_id` 为空并在界面上标成"游离运行"。二选一，建议后者，更诚实。
